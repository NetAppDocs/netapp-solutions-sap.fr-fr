...
+
Le scénario standard pour les systèmes HANA MDC à locataire unique consiste à utiliser des ressources complètes avec restauration de volume. Pour un système HANA MDC comportant plusieurs locataires, vous pourriez souhaiter restaurer un seul locataire. Pour plus d'informations sur la restauration du logement d'un seul locataire, consultez link:hana-sc-generic-restore.html["Restauration et récupération (netapp.com)"^].
