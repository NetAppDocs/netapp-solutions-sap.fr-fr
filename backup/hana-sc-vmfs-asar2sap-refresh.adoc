...
Des informations détaillées sur les opérations d'actualisation du système SAP à l'aide de SnapCenter peuvent être trouvées à  https://docs.netapp.com/us-en/netapp-solutions-sap/lifecycle/sc-copy-clone-introduction.html["Automatisation des opérations de copie et de clonage du système SAP HANA avec SnapCenter"].
...
Pour les versions de SnapCenter < 6.2, il existe des limitations à prendre en compte.
...
Comme mentionné dans la section « Prérequis », pour les versions de SnapCenter < 6.2, le service spl de SnapCenter sur l’hôte HANA doit être redémarré à l’aide de la commande « systemctl restart spl » pour initier un nettoyage approprié.
