...
summary: 'Les nouveaux volumes peuvent désormais être montés sur l"hôte cible, en fonction du fichier /etc/fstab créé précédemment.' 
---
...
link:saphana-dr-anf_create_new_volumes_based_on_snapshot_backups_at_the_disaster_recovery_site.html["Précédent : créez de nouveaux volumes basés sur des sauvegardes Snapshot sur le site de reprise d'activité."]
...
....
link:saphana-dr-anf_hana_database_recovery.html["Suivant : restauration de la base de données HANA."]
