...
+
Si le système n'est plus nécessaire, le processus de nettoyage s'effectue selon les étapes suivantes :

+
.. Exécutez le workflow de suppression de clone SnapCenter pour le volume de données SAP HANA, y compris l'arrêt de la base de données.
+
[NOTE]
====
Le workflow de suppression de clone SnapCenter ne supprime pas l’entrée  `/etc/fstab` par défaut.

Pour que l' `/etc/fstab`entrée soit supprimée automatiquement lors du processus de suppression du clone, le clone doit d'abord être récupéré et devenir une ressource détectée automatiquement. Si cette étape n'est pas effectuée, l' `/etc/fstab`entrée n'est pas supprimée automatiquement et doit être supprimée manuellement.

====
.. Arrêt des services SAP HANA
.. Exécutez le workflow de suppression de clone SnapCenter pour le volume partagé SAP HANA.
