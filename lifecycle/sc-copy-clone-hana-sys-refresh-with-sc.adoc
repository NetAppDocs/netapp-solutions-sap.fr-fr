...
|  | Chiffrement de persistance SAP HANA | SFSS : Initialement, ou si les clés racine ont été modifiées sur le système source, la ou les sauvegardes de clés racine doivent être importées sur le système cible avant que la restauration puisse être exécutée. LSS : Initialement, ou si le mot de passe de sauvegarde de la clé racine a été modifié, le mot de passe doit être défini dans la configuration LSS. 
...
|  | Chiffrement de persistance SAP HANA | SFSS : Si le DBID du système source n’existe pas auparavant sur le système cible, la base de données système doit d'abord être restaurée, avant que la sauvegarde de la clé racine de ce locataire puisse être importée. LSS : La topologie LSS sera automatiquement synchronisée avec la topologie HANA lors de la restauration des bases de données locataires. 
...
Tous les systèmes SAP HANA doivent être configurés conformément au guide de configuration. link:../bp/hana-aff-nfs-introduction.html["SAP HANA sur les systèmes AFF de NetApp avec NFS"]. SnapCenter et les ressources SAP HANA ont été configurés conformément au guide des bonnes pratiques. link:../backup/hana-sc-generic-scope.html["SAP HANA : sauvegarde et restauration avec SnapCenter"].
...
. Configuration du système SAP HANA dans SnapCenter comme décrit dans link:../backup/hana-sc-generic-scope.html["SAP HANA : sauvegarde et restauration avec SnapCenter"]
+
...
Dans une configuration SAN Fibre Channel, vous devez indiquer le protocole utilisé sur l'écran suivant.
