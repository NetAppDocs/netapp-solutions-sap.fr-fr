...
Lorsque le chiffrement de la persistance SAP HANA est activé et, selon le magasin sécurisé utilisé pour les clés racines de chiffrement, instance SFSS ou magasin sécurisé local LSS, des étapes supplémentaires sont nécessaires avant de pouvoir récupérer la base de données SAP HANA sur le système cible.

Avec SFSS, les clés racine sont stockées dans le système de fichiers et doivent être sauvegardées séparément en plus de la base de données HANA. Les sauvegardes des clés racine doivent ensuite être importées sur le système cible pour permettre la restauration de la base de données HANA. Cela doit être fait une seule fois, si les clés racine ne sont pas modifiées.

Avec LSS, les clés racine sont sauvegardées et restaurées en même temps que la base de données HANA en une seule opération. Pour pouvoir récupérer la base de données HANA, vous devez disposer du mot de passe utilisé pour la sauvegarde des clés racine.

Les opérations requises sont décrites plus en détail dans les chapitres suivants.



=== Stockage sécurisé SFSS
...
=== Stockage sécurisé LSS


NOTE: Vous devez utiliser au moins la version 2.00.088 de HANA, sinon la procédure décrite ne fonctionnera pas, voir également la note SAP 3613110.

Pour récupérer un système HANA qui utilise le chiffrement et LSS, vous avez besoin du mot de passe qui a été utilisé pour sauvegarder les clés racine.
...
....
Cette commande permet de définir le mot de passe du système source dans la configuration LSS du système cible. La même instruction doit être utilisée pour la base de données système et la base de données locataire.

Si le mot de passe n'a pas été défini au préalable, vous pouvez le fournir lors de l'opération de récupération.
