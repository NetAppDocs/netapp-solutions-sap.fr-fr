...
---
= Automatisation des opérations de copie de systèmes SAP avec libelle SystemCopy
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Bien que les systèmes de réparation et les systèmes de test de reprise après sinistre soient généralement fournis à l'aide de clones de systèmes SAP (qui ne nécessitent pas d'opérations de post-traitement importantes), pour les systèmes de test et de formation mis à jour, ces étapes de post-traitement doivent être appliquées pour permettre la coexistence avec le système source. Par conséquent, ce document se concentre sur les scénarios de mise à jour des systèmes SAP. Plus de détails sur les différents cas d'utilisation peuvent être trouvés dans le rapport technique link:sc-copy-clone-introduction.html["Automatisation des opérations de copie et de clonage du système SAP HANA avec SnapCenter"^].
