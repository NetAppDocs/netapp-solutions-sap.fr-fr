...
Définissez l'heure de location NFSv4 sur le SVM, comme indiqué dans le tableau suivant si des systèmes à hôtes multiples SAP HANA sont utilisés.
...
|===
Depuis HANA 2.0 SPS4, HANA fournit des paramètres pour contrôler le comportement de basculement. Au lieu de définir l'heure du bail au niveau du SVM, NetApp recommande d'utiliser ces paramètres HANA. Les paramètres sont compris dans `nameserver.ini` comme indiqué dans le tableau suivant. Conservez l'intervalle de relance par défaut de 10 secondes dans ces sections.
