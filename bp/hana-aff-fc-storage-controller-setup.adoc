...
. Créez et configurez des volumes et des LUN dans les agrégats comme décrit dans la section link:hana-aff-fc-host-setup_single_host.html["Configuration d'un seul hôte"] pour hôtes individuels ou en section link:hana-aff-fc-host-setup_multiple_hosts.html["Configuration de plusieurs hôtes"]
