...
Le `/hana/shared` volume doit être accessible par tous les hôtes HANA et est donc exporté via NFS. Même s'il n'existe pas de KPI de performance spécifiques pour le `/hana/shared` système de fichiers, NetApp recommande d'utiliser une connexion Ethernet 10Gb.
...
NOTE: Si le service SAP Local Secure Store (LSS) est utilisé, le `/lss/shared/SID` répertoire doit être accessible depuis tous les hôtes. NetApp recommande de créer un volume pour le `/lss/shared/SID` répertoire et de le monter sur tous les hôtes à l'aide de NFS.
...
Les LUN d'un groupe de volumes LVM doivent être stockées dans un agrégat différent et sur un contrôleur différent. Le tableau suivant présente un exemple pour deux LUN par groupe de volumes dans un système SAP HANA multi-hôtes 2+1.
...
NOTE: L'exemple ci-dessous configure ces options lors de la création du volume
