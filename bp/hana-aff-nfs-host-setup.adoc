...
NOTE: Les exemples suivants montrent une base de données SAP HANA avec SID=NF2 utilisant LSS.
...
L'exemple suivant montre une base de données SAP HANA multi-hôtes avec SID=NF2 utilisant NFSv4.1 pour les systèmes de fichiers de données et de journaux, et NFSv3 pour les  `/hana/shared`, `/lss/shared`, et  `/usr/sap/NF2` systèmes de fichiers. Une taille de transfert NFS de 1MB pour les lectures et de 256k pour les écritures est utilisée.
