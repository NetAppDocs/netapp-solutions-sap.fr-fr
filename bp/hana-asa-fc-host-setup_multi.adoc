...
NOTE: Les systèmes SAP HANA multi-hôtes nécessitent le  `/hana/shared` système de fichiers et les  `/lss/shared` systèmes de fichiers, dans le cas où le service SAP Local Secure Store (LSS) est utilisé, connectés à tous les hôtes d’un système. Il s’agit généralement d’un partage NFS fourni par un serveur NFS. Il est recommandé d’utiliser un serveur NFS à haute disponibilité, par exemple un système NetApp FAS ou AFF. Une autre option consiste à utiliser le serveur NFS intégré d’un hôte LINUX pour cela.
...
Pour monter les systèmes de fichiers, exécutez la commande suivante : `mount -a` commande sur chaque hôte.
