...
`echo options nfs max_session_slots=<calculated value> > /etc/modprobe.d/nfsclient.conf` suivi d'un redémarrage. La valeur côté serveur doit également être ajustée, définissez le nombre d'emplacements de session comme décrit dans link:saphana-fas-nfs_storage_controller_setup.html#nfs-configuration-for-nfsv4["Configuration NFS pour NFSv4."]
