...
summary: 'La configuration requise pour l"installation du logiciel SAP HANA est la suivante :' 
---
...
link:saphana-fas-nfs_sap_hana_data_volume_size.html["Précédent : taille du volume de données SAP HANA."]



== Installez sur un système à hôte unique

L'installation du logiciel SAP HANA ne nécessite aucune préparation supplémentaire pour un système à hôte unique.
...
. À l'aide du système SAP `hdbclm` outil d'installation, démarrez l'installation en exécutant la commande suivante sur l'un des hôtes worker. Utilisez le `addhosts` option permettant d'ajouter le second travailleur (`sapcc-hana-tst-07`) et l'hôte de secours (`sapcc-hana-tst-08`).
+
....
...
link:saphana-fas-nfs_adding_additional_data_volume_partitions.html["Ensuite, ajout de partitions de volume de données supplémentaires."]
