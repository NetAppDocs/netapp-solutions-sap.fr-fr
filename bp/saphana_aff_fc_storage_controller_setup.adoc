...
. Créer un serveur SVM (Storage Virtual machine), comme décrit dans la section « virtual machine configuration."
. Création d'interfaces logiques (LIF), comme décrit dans la section « interface configuration."
. Créez un ensemble de ports, comme décrit dans la section « port sets."
. Créer des groupes initiateurs, des volumes et des LUN au sein de ces agrégats, comme décrit dans la section création de « and volumes and mapping LUNs to initiator groups."
