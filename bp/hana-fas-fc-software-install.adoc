...
....
+
À l'aide de l'outil d'installation SAP `hdblcm`, démarrez l'installation en exécutant la commande suivante sur l'un des hôtes workers. Utilisez `addhosts` l'option pour ajouter le second worker (sapcc-hana-tst-06) et l'hôte de secours (sapcc-hana-tst-07). Le répertoire dans lequel le fichier préparé a `global.ini` été stocké est inclus avec l' `storage_cfg`option CLI (`--storage_cfg=/hana/shared`). Selon la version du système d'exploitation utilisée, il peut être nécessaire d'installer phyton 2.7 avant d'installer la base de données SAP HANA.
