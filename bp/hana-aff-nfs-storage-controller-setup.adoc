...
== Protection autonome contre les ransomwares ONTAP

Les snapshots créés par ONTAP Autonomous Ransomware Protection ne peuvent pas être utilisés pour la restauration avancée de la base de données SAP HANA. Par conséquent, il n'est pas recommandé de les utiliser pour les volumes de données et de journaux SAP HANA et cette fonctionnalité doit être désactivée pour ces volumes. Utilisez plutôt la protection des données basée sur SnapCenter avec des snapshots inviolables pour la protection contre les ransomwares : link:../backup/hana-sc-generic-scope.html["Sauvegarde et restauration SAP HANA avec SnapCenter"]. ONTAP Autonomous Ransomware Protection peut être bénéfique pour d'autres volumes tels que /hana/shared.

Il est possible de désactiver l'activation automatique de cette fonctionnalité lors de la création d'un volume pour un cluster ONTAP, cependant il est recommandé de conserver le paramètre par défaut et de la désactiver pour les volumes où cette fonctionnalité n'est pas nécessaire.
...
NOTE: Si le service SAP Local Secure Store (LSS) est utilisé, le `/lss/shared/SID` répertoire doit être accessible sur tous les hôtes. NetApp recommande de créer un répertoire dans le `SID_shared` volume pour le `/lss/shared/SID` répertoire.
...
| SID_shared | lss-shared | /lss/shared/SID | Monté sur tous les hôtes 
...
| Désactivez la protection contre les ransomwares, sauf pour le volume SID_shared | vol modify -vserver <vserver-name> -volume <volname> -anti-ransomware-state désactivé 
