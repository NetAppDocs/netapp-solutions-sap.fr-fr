...
link:saphana_fas_fc_sap_hana_storage_connector_api.html["Précédent : API du connecteur de stockage SAP HANA."]

Avant de configurer l'hôte, les utilitaires d'hôte SAN NetApp doivent être téléchargés depuis le http://mysupport.netapp.com/["Support NetApp"^] Et installé sur les serveurs HANA. La documentation de l'utilitaire hôte contient des informations sur les logiciels supplémentaires qui doivent être installés en fonction du HBA FCP utilisé.

La documentation contient également des informations sur les configurations multipathing spécifiques à la version de Linux utilisée. Ce document décrit les étapes de configuration requises pour SLES 15 et Red Hat Enterprise Linux 7.6 ou version ultérieure, comme décrit dans https://library.netapp.com/ecm/ecm_download_file/ECMLP2547958["Guide d'installation et de configuration de Linux Host Utilities 7.1"^].
...
NOTE: Les étapes 1 à 6 doivent être effectuées sur tous les hôtes travailleurs et de secours de la configuration SAP HANA à plusieurs hôtes.
...
NOTE: L'exemple de résultat montre le contenu du `/etc/multipath.conf` Fichier, qui inclut des noms d'alias pour les quatre LUN d'un système à plusieurs hôtes 2+1. S'il n'y a pas `multipath.conf` fichier disponible, vous pouvez en créer un en exécutant la commande suivante : `multipath -T > /etc/multipath.conf`.
...
NOTE: Si les commandes ci-dessus ne trouvent pas les volumes, un redémarrage est nécessaire.
...
link:saphana_fas_fc_i_o_stack_configuration_for_sap_hana.html["Next : configuration de la pile d'E/S pour SAP HANA."]
